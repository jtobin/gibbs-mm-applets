...
myRepo = "http://cran.r-project.org"
...
plotter = function(x, z){
    palette = c('red', 'blue', 'green', 'orange', 'cyan')
    plot(x, type = 'n', axes = F, xlab = "", ylab = "", xlim = c(min(x[,1]) - 3, max(x[,1]) + 3), ylim = c(min(x[,2]) - 8, max(x[,2]) + 8))
    if (missing(z)) {points(x = x[,1], y = x[, 2], col = 'black', pch = 16)} else {
    points(x = x[which(z == 1), 1], y = x[which(z == 1), 2], col = palette[1], pch = 16)
    points(x = x[which(z == 2), 1], y = x[which(z == 2), 2], col = palette[2], pch = 16)
    points(x = x[which(z == 3), 1], y = x[which(z == 3), 2], col = palette[3], pch = 16)    
    points(x = x[which(z == 4), 1], y = x[which(z == 4), 2], col = palette[4], pch = 16)
    points(x = x[which(z == 5), 1], y = x[which(z == 5), 2], col = palette[5], pch = 16)}
}
...
################################################################################
# Inference via collapsed Gibbs sampling.  Note that this can be collapsed further, but you will lose the ellipses.
################################################################################
simIterations = 100
...
for (counter in 1:simIterations){
  ...
  for (i in 1:nrow(x)){
    ...
    if (n_deleted[current_obs_index] > 0) n_deleted[z[i,]]  = n_deleted[z[i,]] - 1

    genVector = rep(NA, K)
    for (k in 1:K) { genVector[k] = dmvnorm(x[i,], phi[k,], phiv[[k]]) }
    specProb = (n_deleted + alpha0/K)*genVector

    if (any(specProb > 0)) z[i] = sample(1:K, size = 1, prob = specProb)
  }
  ...
  for (k in 1:K){
    if (n[k] > 0){
      for (i in 1:n[k]){
        if (n[k] == 1) { S[[k]] = S[[k]] + (x[areK[[k]],] - xMean[[k]]) %*% t(x[areK[[k]],] - xMean[[k]]) } else
